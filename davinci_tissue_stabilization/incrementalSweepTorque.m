function [ wmt ] = incrementalSweepTorque( structNeedleGeometry, K, transMatFrmNeedleToTissue, spVelocity, integral_limit )
    syms theta;
...
%     fun =@(theta) r * K * cross(s_t, cross((wedge(spVelocity.angV) * q_t + spVelocity.transV), s_t));
    fun = -r * cross(q_t, K * cross(s_t, cross((wedge(spVelocity.angV) * q_t + spVelocity.transV), s_t)));
    func_handle = matlabFunction(fun);
    
    wmt = integral(func_handle, integral_limit.min, integral_limit.max, 'ArrayValued', true);
end
